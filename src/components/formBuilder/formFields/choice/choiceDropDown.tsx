...
import { CornerDownRightIcon, Plus, TrashIcon } from "lucide-react";
...
export function ChoiceDropDownField({ question, index }: IProps) {
  ...
  const handleOptionBlur = async (idx: number) => {
    ...
    if (originalOption && option.label !== originalOption.label) {
    ...
    <div className="p-6 w-full max-w-3xl mx-auto flex items-baseline gap-3">
      ...
      <div className="space-y-4 w-full flex-1">
        ...
        <div className="mt-6">
          ...
          >
            <Select disabled>
              <SelectTrigger className="w-[300px] cursor-not-allowed opacity-60">
                <SelectValue placeholder="Select an option..." />
              </SelectTrigger>
              <SelectContent>
                {options.length > 0 ? (
                  options.map((option, i) => (
                    <SelectItem key={option.id || `option-${i}`} value={option.value}>
                ...
                >
                  ...
                  {i === options.length - 1 && (
                    <pre className="text-sm text-muted-foreground italic flex items-center gap-3 ml-2">
                      <CornerDownRightIcon size={16} className="-skew-x-16" />
                      Enter to add New
                    </pre>
                  )}
                </motion.div>
